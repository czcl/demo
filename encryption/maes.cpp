...
void CLAes::InitializePrivateKey(DWORD KeySize,UCHAR *KeyBytes)
{
    if(m_lpAes)
    {
        delete m_lpAes;
...
DWORD CLAes::OnAesEncrypt(LPVOID InBuffer,DWORD InLength,LPVOID OutBuffer)
{
    DWORD OutLength=0;
...
DWORD CLAes::OnAesUncrypt(LPVOID InBuffer,DWORD InLength,LPVOID OutBuffer)
{
    DWORD OutLength=0;
    if (m_lpAes==NULL||OutBuffer==NULL)
    {
